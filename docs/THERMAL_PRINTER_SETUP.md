...
| Text runs off the right edge (dialog path) | A non-zero `@page` margin shrinks the page box below the 58mm body, so the overflow is clipped on the right or spilled onto a second strip. Or the driver's paper size is still A4/80mm | Keep `@page { size: 58mm auto; margin: 0; }` and put side spacing in the body padding, which is inside the page box. Set the driver's paper to 58mm roll |
...
- The HTML fallback sheets in both files declare `@page { size: 58mm auto; margin: 0; }`.
